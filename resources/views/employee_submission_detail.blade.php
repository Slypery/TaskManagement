...
@endsection
@section('main')
    <form id="FormEdit" action="{{ route('employee.submissions.update', $task_origin->employee_submission->id) }}" method="post" enctype="multipart/form-data" class="block w-full max-w-[1000px]">
        @csrf
        @method('put')
        <a href="{{ route('employee.task_list.detail', $task_origin->id) }}" class="opacity-50 flex w-min">
        ...
        <x-horizontal-input id="EditTitle" name="title" value="{{ $task_origin->employee_submission->title }}" placeholder="Title" type="text" label="Title" labelSpan="2" />
        <x-text-editor name="description" value="{!! $task_origin->employee_submission->description !!}" label="Decription" />
        <div class="font-semibold mt-3">Attachments</div>
        <div class="w-full grid grid-cols-12 gap-2 mt-2">
            @foreach ($task_origin->employee_submission->attachment as $index => $item)
